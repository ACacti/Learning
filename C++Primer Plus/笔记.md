...
16. 指针和const
    ...
    * const类型指针指向非const类型：指针变量不能被修改，指针指向数据不能被修改
    * const类型指针指向const类型数据：同上
    * 非const类型指针指向const类型数据：C++禁止
    * const int * p: 禁止改变指针指向的数据
    * int * const p: 禁止改变指针指向
...
18. 临时变量、引用参数和const
    ...
    > 如果实参与引用参数不匹配，C++将生成临时变量。当前，进到参数为const引用时，c++才允许这么做。
...
20. 函数模板
    ...
    * ```c++
      ...
      1. 显式实例化
      	直接调用：foo<char>('a');
      	先实例化再调用：
      	template<> void Swap<char> foo(char a);
      	foo('a');
      2. 显式具体化
         追加定义：
         template<> void foo(char a){...} 或
         template<> void foo<char> foo(char a){...}
      ```
...
1. cv-限定符
   ...
   * volatitle
     ...
     > 关键字volatile声明，即使程序代码没有对内存单元进行修改，其值也可能法神变化。
     >
...
```c++
Stockk *pstock = new Stock("Furry Mason", 20, 3.2);
```
...
* 有些情况下，常规枚举将自动转换为整型：如将其赋值给int变量或用于比较表达式时，但作用域内枚举不能隐式转换为整型.
...
* 如果有过个构造函数，则必须以相同的方式使用new,要么都带中括号，要么都不带。因为只有一个析构函数，所有构造函数都必须与它兼容。
...
使用new[]释放buffer指向的内存，将无法自动调用这块内存上JustTesting 对象t的析构函数。这会导致JustingTest构造函数中申请的内存无法释放：
...
* Brass类声明了一个虚析构函数。
  ...
  * 如果基类的析构函数不是虚的，在释放new申请的内存时，则将只调用对应为指针类型的析构函数。
  * 如果使用一个基类指针指向一个动态申请的派生类内存，则将只调用基类的析构函数，尽管指针指向的是一块派生类内存。
